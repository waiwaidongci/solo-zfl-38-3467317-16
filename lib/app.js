  ...
  const server = http.createServer(async (req, res) => {
    try {
      ...
      if (byNoMatch && req.method === "GET") {
        const ropeNo = decodeURIComponent(byNoMatch[1]);
        const rope = findByRopeNo(store.db, ropeNo);
        if (!rope) return send(res, 404, { error: "rope_no_not_found", ropeNo });
      ...
      if (oneMatch && req.method === "GET") {
        ...
        if (findByRopeNo(store.db, key)) {
          return send(res, 400, { error: "use_internal_id", ropeNo: key, message: "请使用 /api/ropes/by-rope-no/ 按索号查询" });
        }
