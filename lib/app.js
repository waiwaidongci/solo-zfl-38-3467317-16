...
// 启动期配置错误：进程必须明确失败退出，而不是带着失效的保护继续运行。
export class ConfigError extends Error {}

const DEFAULT_MAX_BODY_BYTES = 1024 * 1024; // 1 MiB

// 请求体上限必须是非负安全整数。
// 仅当选项与环境变量都“未设置”（undefined / 缺失 / 空字符串）时才使用默认值；
// 一旦显式提供但非法，立即抛出，绝不静默退化为无上限：
//   - createApp({ maxBodyBytes })：必须本身就是非负安全整数（不做字符串转换）；
//   - KNOT_MAX_BODY_BYTES（环境变量恒为字符串）：只接受规范的非负整数写法，
//     非数字、小数、负数、带空格等一律拒绝（避免 Number("abc") => NaN 关闭保护）。
function resolveMaxBodyBytes(optionValue) {
  const envRaw = process.env.KNOT_MAX_BODY_BYTES;
  const optionSet = optionValue !== undefined;
  const envSet = envRaw !== undefined && envRaw !== "";

  if (!optionSet && !envSet) return DEFAULT_MAX_BODY_BYTES;

  if (optionSet) {
    if (typeof optionValue !== "number" || !Number.isSafeInteger(optionValue) || optionValue < 0) {
      throw new ConfigError(
        `createApp({ maxBodyBytes }) 必须是非负安全整数（字节），当前为 ${typeof optionValue} ${String(optionValue)}`
      );
    }
    return optionValue;
  }

  // 环境变量：严格匹配规范十进制整数（可选前后不许空格），再做安全整数范围校验
  if (!/^\d+$/.test(envRaw)) {
    throw new ConfigError(`KNOT_MAX_BODY_BYTES 必须是非负整数（字节），当前为 ${JSON.stringify(envRaw)}`);
  }
  const parsed = Number(envRaw);
  if (!Number.isSafeInteger(parsed)) {
    throw new ConfigError(`KNOT_MAX_BODY_BYTES 超出安全整数范围，当前为 ${JSON.stringify(envRaw)}`);
  }
  return parsed;
}
...
export function createApp(options = {}) {
  ...
  // 请求体大小上限（字节）：默认 1MiB；非法配置在启动期即失败。
  ...
  function send(res, status, data, closeConnection = false) {
    ...
    if (closeConnection) {
      ...
      if (!sock) return;
      // 响应写完即优雅半关闭（发 FIN）：客户端读完 413 会立刻收到连接结束，
      // 不引入固定等待；对仍在灌入的攻击端，由内核背压/随后 RST 释放，服务端
      // 不保留任何定时句柄。
      res.once("finish", () => {
